...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, onLogout, onOpenHelp, currentUser, isOpen, onClose, permissions, socialUnreadCount, evaluationNotifCount }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-[#EBEBEB] dark:bg-[#19324B] text-slate-600 dark:text-slate-300 h-screen shadow-xl border-r border-slate-200 dark:border-slate-700 transition-transform duration-300 ease-in-out flex flex-col
        ${isOpen ? 'translate-x-0' : '-translate-x-full'}
        md:translate-x-0 md:static
      `}>
        {/* Header - Fixed at top */}
        <div 
          className="shrink-0 p-6 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-[#EBEBEB] dark:bg-[#19324B] transition-colors duration-200 cursor-pointer hover:bg-slate-200 dark:hover:bg-[#284664]/50"
          onClick={() => handleNavClick('home')}
        >
          <div className="flex items-center gap-3">
            <div className="bg-[#EBEBEB] dark:bg-[#19324B] p-2 rounded-lg border border-slate-200 dark:border-slate-700">
        ...
        <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
            ...
            <div className="shrink-0 p-4 border-t border-slate-200 dark:border-slate-700 bg-[#EBEBEB] dark:bg-[#19324B]/50 transition-colors duration-200 space-y-3">
              ...
              <div className="border-t border-slate-300 dark:border-slate-700 pt-3">
                <div className="flex items-center gap-3 px-2 mb-4">
                    {currentUser?.avatar ? (
                        ...
                        <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-[#284664] flex items-center justify-center font-bold text-xs text-slate-600 dark:text-slate-300">
