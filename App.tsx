  ...
  const unflattenData = (table: string, data: any): any => {
    ...
    if (table === 'staff') {
      return {
        ...
        role: (d('role') === 'Security' || d('role') === 'Segurança' || d('role') === 'Ronda' ? 'Ronda' : 
               d('role') === 'Concierge' || d('role') === 'Portaria' || d('role') === 'Porteiro' || d('role') === 'Controlador de Acesso' ? 'Controlador de Acesso' : 
               d('role') || 'Setor'), 
        sector: d('sector') || '', 
